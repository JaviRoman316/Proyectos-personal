...
os.chdir(directorio)
from funciones_optimizacion_rutas import *

# Paths
path_carpeta = r'G:\Unidades compartidas\Medicamentos del Bajío - APN Bajío\2. Áreas\Embarques\Sprint 1 PRO-DIS-01 Planeación de rutas\05 Optimización de rutas'
...
gdf_ubicaciones = csv_a_gdf(df_ubicaciones)

# Cargamos la matriz de distancias y tiempos entre todos los puntos
df_matriz_rutas = pd.read_csv(os.path.join(path_inputs, 'matriz_rutas.csv'), encoding=encoding)

id_ubicaciones = gdf_ubicaciones['id_bajio'].tolist()
nom_ubicaciones = gdf_ubicaciones['nombre'].tolist()
dict_ubicaciones = dict(zip(nom_ubicaciones, id_ubicaciones))
dict_ubicaciones['CEDIS'] = 'PE000'

# Cambio de variables
df_matriz_rutas['origen'] = df_matriz_rutas['origen'].map(dict_ubicaciones)
df_matriz_rutas['destino'] = df_matriz_rutas['destino'].map(dict_ubicaciones)
gdf_ubicaciones['nombre'] = gdf_ubicaciones['nombre'].map(dict_ubicaciones)

# Creacion de los inputs del moedlo
distancias = df_matriz_rutas.drop(columns=['duracion_min']).pivot(index='origen',columns='destino',values='distancia_ruta_km').to_numpy()
tiempos = df_matriz_rutas.drop(columns=['distancia_ruta_km']).pivot(index='origen',columns='destino',values='duracion_min').to_numpy()
id_ubicaciones = df_matriz_rutas.drop(columns=['duracion_min']).pivot(index='origen',columns='destino',values='distancia_ruta_km').columns.tolist()
...
auxiliar = df_matriz_rutas.drop(columns=['duracion_min']).pivot(index='origen',columns='destino',values='distancia_ruta_km').columns.tolist()
auxiliar_2 = range(0,197)
...
######
file_name = 'rutas_optimas_osm.csv'
